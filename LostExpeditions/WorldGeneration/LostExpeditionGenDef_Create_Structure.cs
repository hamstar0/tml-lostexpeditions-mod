	...
	public partial class LostExpeditionGenDef {
		public static bool CreateExpeditionStructure(
			...
			for( int i = leftTileX; i < toTileX; i++ ) {
				LostExpeditionGenDef.PaveCampAt( i, nearFloorTileY, paveTileType );
			}
		...
		private static void PaveCampAt( int tileX, int nearFloorTileY, int tileType ) {
			...
			for( Tile tile = Framing.GetTileSafely(tileX, solidY);
						solidY < maxSolidY && !LostExpeditionGenDef.IsValidFloorTile(tile);
			...
			for( int i=needsFill.Count-1; i>=0; i-- ) {
				...
				if( !LostExpeditionGenDef.IsValidFloorTile(tile) ) {
					tile.ClearEverything();
